...
5. Next its important to add a file, sripts/token-setup.js to the project. This fills in a step URDEX has already taken, hence it doesn't need this file. Before tokens can move, they must be approved - a task token-setup accomplishes.

Once scripts/token-setup.js is added, paste the following and save. 
...
6. Now run scripts/token-setup.js:
```bash
